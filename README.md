...
4. Store the generated scripts under `dist/<tag>`, copy them into the Razor Class Library's `wwwroot`, and pack `LegacyBlazorJs` using the upstream tag without its `v` prefix. For example, tag `v8.0.27` produces `dist/v8.0.27` and package version `8.0.27`.
...
This resolves and builds the latest stable .NET 8 tag, stores its JavaScript files in `dist/<tag>`, and creates `artifacts/packages/LegacyBlazorJs.<version>.nupkg`. Set an explicit tag for a reproducible historical build:
